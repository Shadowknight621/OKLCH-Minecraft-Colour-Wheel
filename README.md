...
The colour wheels show lightnesses of 35%, 60%, and 85%, respectively, with each block plotted to the closest colour wheel. Very low chromas (<.05 chroma) are omitted from the colour wheels, as are colours outside of a P3 gamut (the reason for the irregular shapes).
...
Leaves, leaf litter, and grass are positioned based on their colour in a forest biome, since colour changes in different biomes.

Most blocks only show one face to remove unnecessary clutter, but blocks with more unique faces often show multiple.
In addition, a number of more detailed blocks such as some workstations are not shown, since the average colour does not effectively mirror how the block appears.
